...
class DMRC(nn.Module):
    def __init__(self,
        ...
        self.att_conv = nn.Conv3d(input_channels, output_channels, kernel_size=1, stride=1)
    ...
    def forward(self, x):
        ...
        ch_att = self.fc(self.avg_pool(output).squeeze()).unsqueeze(dim=-1).unsqueeze(dim=-1).unsqueeze(dim=-1)
...
class DMSC(nn.Module):
    def __init__(self,
        ...
        self.conv2 = nn.Sequential(
                    ...
                    #nn.Conv3d(input_channels, output_channels, kernel_size=7, stride=1, padding=3),
                    nn.Conv3d(output_channels, output_channels, kernel_size=7, stride=1, padding=3, groups=output_channels),
                    nn.LeakyReLU(negative_slope=0.2, inplace=True),
                    nn.BatchNorm3d(output_channels),
                    #nn.Conv3d(output_channels, output_channels, kernel_size=7, stride=1, padding=3),
                    nn.Conv3d(output_channels, output_channels, kernel_size=7, stride=1, padding=3, groups=output_channels),
                    nn.LeakyReLU(negative_slope=0.2, inplace=True),
                    nn.BatchNorm3d(output_channels)
                    )
        ...
        self.conv_atten = nn.Sequential(
            nn.Conv3d(output_channels * 2, output_channels * 2, kernel_size=1, bias=False),
            nn.Sigmoid()
        )
        self.conv_redu = nn.Conv3d(output_channels * 2, output_channels, kernel_size=1, bias=False)
